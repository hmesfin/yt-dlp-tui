...
def test_progress_templates_are_attached_with_defaults():
  ...
  # Every interpolated field must carry a NON-EMPTY |default. A bare `|` is not
  # enough: for a None value yt-dlp substitutes the default as a raw literal and
  # skips the `j` conversion, so `%(info.title|)j` renders `"title":` — invalid
  # JSON. String fields need `|""`; numeric fields are fine with `|0`.
  for tpl in (download_tpl, pp_tpl):
    for field in tpl.split("%(")[1:]:
      key_and_default = field.split(")")[0]
      assert "|" in key_and_default
      assert key_and_default.split("|", 1)[1] != ""
...
def _progress(payload: str) -> ProgressEvent:
  ...
  if not isinstance(d, dict):
    # json.loads accepts scalars and lists; `.get` on those raises
    # AttributeError, which the caller does not catch.
...
- Produces: `async def run(argv: list[str]) -> AsyncIterator[Event]` — yields events, always terminating with exactly one `DoneEvent`. Closing the iterator SIGTERMs the child's whole process group (escalating to SIGKILL), so ffmpeg and aria2c helpers are not orphaned.
...
async def test_closing_the_iterator_terminates_the_child_promptly():
  ...
  assert isinstance(first, LogEvent) and first.text == "up"
...
> **Shipped implementation diverges from the block above — read `src/yt_dlp_tui/runner.py`, not this listing.** The sketch above has three confirmed defects: `create_task(gather(...))` raises `TypeError`; `proc.wait()` never resolves while a grandchild holds the inherited pipes (so no `DoneEvent` is ever yielded); and `terminate()` on the child alone orphans ffmpeg. The shipped version uses a `TaskGroup` + sentinel queue, a watchdog polling `proc.returncode`, and process-group signalling.
...
def test_non_object_json_returns_not_ok():
...
def test_junk_field_types_degrade_instead_of_raising():
...
def parse_probe_json(raw: str) -> ProbeResult:
  ...
  return ProbeResult(
    ...
    playlist_count=_as_int(d.get("playlist_count")),
    ok=True,
  )
...
Note: `action_advanced` and `action_download` are deliberate stubs. Tasks 8 and
9 replace each body and add a top-of-file import for the screen it pushes.
There is no circular import to work around — `advanced.py` and `run.py` never
import `main.py` or `app.py`, so a normal top-level import is correct once those
modules exist. Do not use function-level imports here.
...
- [ ] **Step 4: Wire the drawer into `main.py`**

Add `from yt_dlp_tui.screens.advanced import AdvancedScreen` to the top-level imports of `screens/main.py`, then replace the `action_advanced` stub body with:
...
- [ ] **Step 4: Wire the run screen into `main.py`**

Add `from yt_dlp_tui.screens.run import RunScreen` to the top-level imports of `screens/main.py`, then replace the `action_download` stub body with:
...
**Task sequencing.** Task 7 ships `action_advanced` and `action_download` as
stubs because the screens they push do not exist yet; Tasks 8 and 9 each replace
one stub body and add a normal top-of-file import. No function-level imports are
used anywhere — `advanced.py` and `run.py` never import `main.py` or `app.py`,
so there is no import cycle to work around. The imports-at-top constraint holds
at every commit.
